...
사용자가 자연어로 작업을 지시하면 Doosan **M0609** 협동로봇과 **OnRobot RG2** 그리퍼가
물체를 인식하고 안전한 경로를 계획해 집어 옮기는 시스템입니다.

사용자의 의도를 해석하는 **VLA 판단 계층**과 실제 로봇의 인식·경로계획·파지를 담당하는
**로봇 제어 계층**으로 구성됩니다. VLM은 대상·개수·목적지를 결정하고, 안전 FSM은 로봇의 이동과 정지,
그리퍼 제어 및 작업 실패 복구를 담당합니다.

> **핵심 원칙:** LLM은 로봇의 좌표, 관절값 또는 그리퍼를 직접 제어하지 않습니다.
> 모든 실제 동작은 안전 FSM의 검증을 거쳐 실행됩니다.
...
두 계층은 JSON 작업 명령으로만 연결됩니다. LLM은 좌표·관절값·그리퍼·승인·정지를 직접 제어하지 않으며,
로봇 계층은 VLA 없이도 독립적으로 계획하고 검증할 수 있습니다.
...
> cuMotion·nvblox 연결과 반복 재계획은 프로토타입입니다. 움직이는 장애물을 대상으로 한
> 실행 중 회피는 충분한 실기 반복 검증을 완료하지 않았으며, 구현과 실기 검증을 같은 의미로 사용하지 않습니다.
